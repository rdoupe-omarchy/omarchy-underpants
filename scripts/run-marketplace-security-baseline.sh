...
# security audit. See RUN-BASELINE.md.
set -euo pipefail
...
Examples:
  ...
  scripts/run-marketplace-security-baseline.sh --local . --sha HEAD
EOF
}
...
# Official workflow metadata (writeValidationMetadata) plus entryPoints on
# each listedPlugins row so a local rerun matches both the bot JSON and the
