...
- Resolve installer and wrapper tools from an allowlisted absolute directory list (`/usr/bin:/bin`) with a minimal closed environment; never search ambient PATH for `python3`, `omarchy`, `pgrep`, `sleep`, or `omarchy-toggle-enabled`. Production resolvers ignore `UNDERPANTS_TRUSTED_PATH`; extra directories are explicit test parameters only.
- Overlay QML launches `/usr/bin/python3` with `clearEnvironment` and an allowlisted session environment (`PATH=/usr/bin:/bin`; no ambient `PYTHON*` / `LD_*`). Session stop files are created under the launcher PID’s held directory fd so children can still resolve them. Local helper output is size-capped while reading, and `hyprctl` / `xdg-terminal-exec` resolve from `/usr/bin:/bin`. Wrapper tool identities bake the validated realpath.
