...
- Overlay QML launches `/usr/bin/python3` rather than a PATH-selected interpreter. Session stop files are created under the held runtime-directory fd. Local helper output is size-capped while reading, and `hyprctl` / `xdg-terminal-exec` resolve from `/usr/bin:/bin`.
